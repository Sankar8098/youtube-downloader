...
TOKEN = '6750957059:AAE5c3xc-OAzsaD7gjIDq8DmV5dl3mG31bk'
...
async def fetch_video_info(update: Update, context: CallbackContext) -> None:
    if update.message:
        ...
        try:
            ...
            await fetching_message.edit_text(f"Failed to fetch video info: {str(e)}")
    else:
        await update.callback_query.answer("Invalid input. Please send a YouTube link.", show_alert=True)
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    if not yt:
        await query.edit_message_text("Video info not found. Please try again.")
        return
    ...
    if query.data == 'video':
        ...
        if not unique_video_streams:
            await query.edit_message_text("No video streams available.")
            return
        ...
        if not audio_streams:
            await query.edit_message_text("No audio streams available.")
            return
        ...
        if not stream:
            await query.edit_message_text("Stream not found. Please try again.")
            return
